...
type
  ...
  TfrmDefault = class(TForm)
    ...
    procedure TFieldGetFoneFormat(Sender: TField; var Text: string; DisplayText: Boolean);
    procedure TFieldGetCPFCNPJFormat(Sender: TField; var Text: string; DisplayText: Boolean);
    procedure TFieldGetIEFormat(Sender: TField; var Text: string; DisplayText: Boolean);
    procedure TFieldGetCNAEFormat(Sender: TField; var Text: string; DisplayText: Boolean);
    procedure TFieldGetCEPFormat(Sender: TField; var Text: string; DisplayText: Boolean);
    procedure TFieldGetNCMFormat(Sender: TField; var Text: string; DisplayText: Boolean);
    procedure TFieldGetCESTFormat(Sender: TField; var Text: string; DisplayText: Boolean);
  ...
  public
    ...
    property ParamSelecao: TParametrosSelecao read FParamSelecao write FParamSelecao;
  end;
...
begin
  ...
  begin
    ...
    case ALength of
      ...
      1:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 11;
      2:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 10;
      3:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 9;
      4:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 7;
      5:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 6;
      6:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 5;
      7:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 3;
      8:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 2;
      9:
        ...
        begin
          if VarIsNull(TJvDBMaskEdit(Sender).EditText) then
            ...
            TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText));
        end;
    end;
  end
  else
  begin
    ...
    case ALength of
      0:
        TJvDBMaskEdit(Sender).SelStart := 1;
      1:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 15;
      2:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 14;
      3:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 12;
      4:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 11;
      5:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 10;
      6:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 8;
      7:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 7;
      8:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 6;
      9:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 4;
      10:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 3;
      11:
        TJvDBMaskEdit(Sender).SelStart := Length(Trim(TJvDBMaskEdit(Sender).EditText)) - 2;
...
procedure TfrmDefault.TFieldGetFoneFormat(Sender: TField; var Text: string; DisplayText: Boolean);
begin
  Text := FormatarFone(Sender.AsString);
end;

procedure TfrmDefault.TFieldGetCPFCNPJFormat(Sender: TField; var Text: string; DisplayText: Boolean);
begin
  Text := FormatarCNPJCPF(Sender.AsString);
end;

procedure TfrmDefault.TFieldGetIEFormat(Sender: TField; var Text: string; DisplayText: Boolean);
begin
  Text := FormatarInscEstadual(Sender.AsString, Sender.DataSet.FieldByName('ID_CIDADES_UF').AsString);
end;

procedure TfrmDefault.TFieldGetNCMFormat(Sender: TField; var Text: string; DisplayText: Boolean);
begin
    ...
    6:
      ...
      begin
        if VarIsNull(TJvDBMaskEdit(Sender).EditText) then
          ...
          TJvDBMaskEdit(Sender).SelStart := Length(TJvDBMaskEdit(Sender).EditText);
      end;
  end;
end;
...
procedure TfrmDefault.TFieldGetCEPFormat(Sender: TField; var Text: string; DisplayText: Boolean);
begin
  Text := FormatarTCCC(Sender.AsString);
end;

procedure TfrmDefault.TFieldGetCESTFormat(Sender: TField; var Text: string; DisplayText: Boolean);
begin
  Text := FormatarCEST(Sender.AsString);
end;

procedure TfrmDefault.TFieldGetCNAEFormat(Sender: TField; var Text: string; DisplayText: Boolean);
begin
...
begin
  ...
  if (Screen.Width = 800) and (Screen.Height = 600) and (Self.Height >= 540) then
  begin
...
procedure TfrmDefault.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
