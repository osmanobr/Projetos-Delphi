          ...
          if($qrempresa)
            {   //a empresa está ok  
                foreach($qrempresa as $rowempresa)
                {
                   ...
                   $celular_empresa = $rowempresa['celular'];   
                }
            }
                        ...
                        if($qratendimento)
                        {
                            foreach ($qratendimento as $rowatendimento)
                                {  $id_atendimento    = $rowatendimento['id'];
                                   ...
                                   if((int)$id_empresa > 0 )
                                       {
                                           $query  = "INSERT INTO tb_chat ( id_empresa, id_atendimento, de, para, tipo, msg, status)VALUES ( $id_empresa, $id_atendente, $de, $para, '$tipo', '$msg','1')";
...
//resolver depois, isso apaga registros que estão vindo duplicados    
$sqlatendimento = $pdo->query("delete a FROM tb_chat AS a, tb_chat AS b WHERE (a.msg=b.msg and a.de = b.de and a.para = b.para) AND a.id < b.id");
    }
