...
unit Dal.ERP.import_produtos;

interface
uses
  System.SysUtils, System.Classes, Data.DB, ListError, Parameters,
  System.Generics.Defaults, System.Generics.Collections, Utilitarios,
  ModelBase, ModelBaseList, Model.ERP.import_produtos, DalBaseCRUD,
  SQLQuery;

type
  TDalERPimportprodutos = class(TDalBaseCRUD)
  private 
...
function TDalERPimportprodutos.Delete(Model: TModelBase): Boolean; 
begin                                                                   
  ...
  Self.FModel := ( Model as TModelERPimportprodutos );              
...
function TDalERPimportprodutos.Insert(Model: TModelBase): Boolean; 
...
begin                                                                    
  ...
  SQL.Insert(import_produtos_alterados).Fields( Self.FModel.ToString ).Values; 
     ...
     SQL.Field(Self.FModel.metade_percentual).Next;
     SQL.Field(Self.FModel.trib).Next;
  dbInsert;
  Result := (ListError.Count = 0); 
end; 

function TDalERPimportprodutos.Select(Model: TModelBase; 
...
begin                                                          
  ...
  SQL.SelectFrom(import_produtos_alterados);
  ...
  begin                                        
    ...
    begin                            
      I := Self.FModelList.Add( TModelERPimportprodutos.Create ); 
      ...
      Self.FModelList[I].metade_percentual      := SQL.getInteger(metade_percentual);
...
begin                                                                    
  ...
  SQL.Update(import_produtos_alterados);
