...
uses
  ...
  FireDAC.VCLUI.Wait, Data.DB, FireDAC.Comp.Client, FireDAC.Phys.PG, FireDAC.Phys.PGDef, FireDAC.Stan.Param, FireDAC.DatS, FireDAC.DApt.Intf, FireDAC.DApt, FireDAC.Comp.DataSet, Vcl.Samples.Gauges;
...
type
  TFrmVIaGeraMVC = class(TForm)
    ...
    MDFM: TMemo;
    Panel6: TPanel;
    BtnGerarParaTabela: TBitBtn;
    BtnGerarSistemaCompleto: TBitBtn;
    Gauge1: TGauge;
    Label1: TLabel;
    Label3: TLabel;
    Label4: TLabel;
    ...
    procedure BtnGerarParaTabelaClick(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure BtnGerarSistemaCompletoClick(Sender: TObject);
  private
...
var
  ...
  var Diretorio : string='d:\';
      ProjetoDPR, UnidadeMenu:TstringList;
...
begin
  result := 'indefinido' ;
  case AnsiIndexStr(UpperCase(texto), ['NUMERIC', 'CHARACTER VARYING','INTEGER', 'TEXTO', 'BOOLEAN','TIMESTAMP', 'DATE', 'INT2', 'INT4']) of
    0 : result := 'Double';
    1 : result := 'WideString';
    2 : result := 'Integer';
    3 : result := 'WideString';
...
procedure TFrmVIaGeraMVC.BtnGerarParaTabelaClick(Sender: TObject);
begin
    EditModel.Text      := 'Model.ERP.'+FilterChars(CBTabelas.Text,['A'..'Z', 'a'..'z', '_', '-']);
    EditView.Text       := 'View.ERP.'+FilterChars(CBTabelas.Text,['A'..'Z', 'a'..'z', '_', '-']);
    EditController.Text := 'Controller.ERP.'+FilterChars(CBTabelas.Text,['A'..'Z', 'a'..'z', '_', '-']);
    EditDal.Text        := 'Dal.ERP.'+FilterChars(CBTabelas.Text,['A'..'Z', 'a'..'z', '_', '-']);
end;
...
procedure TFrmVIaGeraMVC.BtnGerarSistemaCompletoClick(Sender: TObject);
var i:integer;
begin
  ProjetoDPR  := TStringList.Create;
  UnidadeMenu := TStringList.Create;
        ProjetoDPR.Clear;
        ProjetoDPR.add('      //Criando o projeto dpr ');
        ProjetoDPR.add('program ERP; ');
        ProjetoDPR.add('uses  ');
        ProjetoDPR.add('  Vcl.Forms, ');
        ProjetoDPR.add('  UnitMenuMae in ''UnitMenuMae.pas'' {FormMenuMaeERP}, ');
  for I := 0 to CBTabelas.Items.Count -1 do
    begin
      CBTabelas.ItemIndex := i;
      SBGerarModulos.Click;
      BtnGerarParaTabela.Click;
      MModel.Lines.SaveToFile(Diretorio+'\Model\'+EditModel.Text+'.pas');
      MController.Lines.SaveToFile(Diretorio+'\Controller\'+EditController.Text+'.pas');
      MDal.Lines.SaveToFile(Diretorio+'\Dal\'+EditDal.Text+'.pas');
      MView.Lines.SaveToFile(Diretorio+'\View\'+EditView.Text+'.pas');
      MDFM.Lines.SaveToFile(Diretorio+'\View\'+'\'+EditView.text+'.dfm');

        //adicionando as unidades no projeto dpr
      ProjetoDPR.add('  '+Editview.Text+' in ''view/'+Editview.Text+'.pas'' {Form1},  ');
      ...
      Label4.Caption := 'Progresso: '+inttostr(i);
      Label4.refresh;
      Label4.repaint;
...
{$R *.res}
//
        ProjetoDPR.add(' begin  ');
        ProjetoDPR.add('  Application.Initialize; ');
        ProjetoDPR.add('  Application.MainFormOnTaskbar := True; ');
        ProjetoDPR.add('  Application.CreateForm(TFormMenuMaeERP, FormMenuMaeERP);');
        ProjetoDPR.add('  Application.Run; ');
        ProjetoDPR.add('end.');
        ProjetoDPR.SaveToFile(Diretorio+'\ERP.dpr');
...
procedure TFrmVIaGeraMVC.FormCreate(Sender: TObject);
...
begin
    ...
    if not DirectoryExists(Diretorio+'\Model') then
        ForceDirectories(Diretorio+'\Model');

    if not DirectoryExists(Diretorio+'\Controller') then
        ForceDirectories(Diretorio+'\Controller');

    if not DirectoryExists(Diretorio+'\View') then
        ForceDirectories(Diretorio+'\View');

    if not DirectoryExists(Diretorio+'\Dal') then
        ForceDirectories(Diretorio+'\Dal');
...
begin
  ...
  Gauge1.Progress := 0;
  Gauge1.MaxValue := CBTabelas.Items.Count-1;
  Label1.Caption := '1';
  Label3.Caption := inttostr(CBTabelas.Items.Count-1);
